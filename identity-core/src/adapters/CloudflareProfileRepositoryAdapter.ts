...
export interface CloudflareBindings {
  readonly table: { execute: (query: string, params?: unknown[]) => Promise<{ results?: unknown[] }> };
  readonly kv?: { get: (key: string) => Promise<string | null>; put: (key: string, value: string) => Promise<void> };
}

export class CloudflareProfileRepositoryAdapter implements IIdentityRepository {
  constructor(private readonly bindings: CloudflareBindings, private readonly logger: ILogger) {}
  ...
  async save(state: IdentityStateProps): Promise<void> {
    ...
    await this.bindings.table.execute('INSERT OR REPLACE INTO profiles (profile_id, data) VALUES (?, ?)', [
      state.metadata.profileId,
      payload
    ]);
    await this.bindings.kv?.put(`identity:${state.metadata.profileId}`, payload);
    this.logger.info('Persisted profile to Cloudflare', { profileId: state.metadata.profileId });
  }

  async delete(profileId: string): Promise<void> {
    await this.bindings.table.execute('DELETE FROM profiles WHERE profile_id = ?', [profileId]);
