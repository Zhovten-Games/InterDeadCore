...
export class IdentitySyncService {
  ...
  async pull(profileId: string, metadataFactory: () => ProfileMetadata): Promise<IdentityAggregate> {
  ...
  async pushSnapshots(profileId: string, snapshots: ScaleSnapshotRef[], options: SyncProfileOptions = {}): Promise<void> {
    if (!options.allowScaleSync || !this.scaleAdapter) {
      this.logger.warn('Scale adapter is disabled; skipping snapshot sync', { profileId });
      return;
    }
    for (const snapshot of snapshots) {
      await this.scaleAdapter.attachSnapshot(profileId, snapshot);
    }
    this.logger.info('Synced scale snapshots', { profileId, count: snapshots.length });
  }
}
